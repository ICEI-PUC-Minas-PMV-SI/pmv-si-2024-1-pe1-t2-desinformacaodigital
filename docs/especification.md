...
|RF-007 | A aplicação deve conter a página “passeio tecnológico” a qual haverá três fases em que o usuário aprenderá brevemente sobre o funcionamento de aparelhos digitais, a internet, golpes e redes sociais com um quiz no final de cada fase. O usuário será pontuado de acordo com os acertos. | ALTA   
|RF-008 | A aplicação deve fornecer um feedback para ações do usuário como confirmações de envio e mensagens de erro. | ALTA
|RF-009 | A aplicação deve conter uma barra de pesquisa onde serão listados interesses pré-definidos com base no público-alvo. | ALTA
